...
with open(args.path+'/coordinates.txt') as f:
    for line in f:
        ...
        start_points.append(
            [round(float(row[2])*100.0/img_size[0]), round(float(row[1])*100.0/img_size[1])])
        # TODO:TIMES 2!
        occl_sizes.append([max(1, round((float(row[8])-float(row[2]))*100.0/img_size[0]))
                           * 2, max(1, round((float(row[7])-float(row[1]))*100.0/img_size[1]))*2])
        count += 1
...
for i in range(len(imgs)):
    ...
    gen_img_deprocessed = draw_arrow3(deprocess_image(imgs[i]), min(
