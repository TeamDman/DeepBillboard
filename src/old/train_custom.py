...
parser.add_argument('-tr', '--transformation', help="realistic transformation type",
                    choices=['light', 'occl', 'blackout'], default='occl')
parser.add_argument('-diff', '--weight_diff',
                    help="weight hyperparm to control differential behavior", default=1, type=float)
parser.add_argument('-w', '--weight_nc',
                    help="weight hyperparm to control neuron coverage", default=0.1, type=float)
parser.add_argument(
    '-st', '--step', help="step size of gradient descent", default=5, type=float)
parser.add_argument(
    '-sd', '--seeds', help="number of seeds of input", default=1, type=int)
parser.add_argument('-grad', '--grad_iterations',
                    help="number of iterations of gradient descent", default=25, type=int)
parser.add_argument('-th', '--threshold',
                    help="threshold for determining neuron activated", default=0, type=float)
parser.add_argument('-t', '--target_model', help="target model that we want it predicts differently",
                    choices=[0, 1, 2], default=0, type=int)
parser.add_argument('-sp', '--start_point',
                    help="occlusion upper left corner coordinate", default=(50, 50), type=tuple)
parser.add_argument('-occl_size', '--occlusion_size',
                    help="occlusion size", default=(50, 50), type=tuple)
parser.add_argument('-overlap_stra', '--overlap_stratage', help='max:select maximum gradient value. sum:..., highest: detect the influence',
                    choices=['max', 'sum', 'highest_influence'], default='sum')
parser.add_argument('-greedy_stra', '--greedy_stratage', help='random_fix:fix pixels if it is changed in one iteration and the img is selected randomly. dynamic:pixels can be changed in each iteration. highest: detect the influence',
                    choices=['random_fix', 'sequence_fix', 'dynamic', 'hightest_fix'], default='dynamic')
parser.add_argument('-fix_p', '--fix_p',
                    help='parameter p(percetage) of fixed images(batches), only p of 1 images will be selected', default=1.0, type=float)
parser.add_argument('-jsma', '--jsma',
                    help='Using jsma or not', default=False, type=bool)
parser.add_argument('-jsma_n', '--jsma_n',
                    help='parameter n of jsma, top n pixels in gradients of board will be selected', default=5, type=int)
parser.add_argument('-sa', '--simulated_annealing',
                    help='Simulated annealing to control the logo update', default=True, type=bool)
parser.add_argument(
    '-sa_k', '--sa_k', help='parameter k of simulated annealing, higher = less changing probability', default=30, type=int)
parser.add_argument(
    '-sa_b', '--sa_b', help='parameter b in range (0,1) of simulated annealing, higher = higher changing probability. p = pow(e,k*diff/pow(b,iter))', default=0.96, type=float)
parser.add_argument('-gpu', '--gpu', default=0, type=int)
parser.add_argument('-path', '--path',
                    default="../Digital/digital_Dave_curve1", type=str)
...
model1 = Dave_orig(input_tensor=input_tensor, load_weights=False)
...
for f in sorted(filelist):
    ...
    input_img_data = cv2.resize(input_img_data, dsize=(img_cols, img_rows))
...
with open(args.path+'/coordinates.txt') as f:
    for line in f:
        ...
        start_points.append(
            [round(float(row[2])*img_cols/img_size[0]), round(float(row[1])*img_rows/img_size[1])])
        # TODO:TIMES 2!
        occl_sizes.append([max(1, round((float(row[8])-float(row[2]))*img_cols/img_size[0]))
                           * 2, max(1, round((float(row[7])-float(row[1]))*img_rows/img_size[1]))*2])
        count += 1
...
for iters in range(args.grad_iterations):
    ...
    for i in range(0, len(imgs), batch):
        ...
        for gen_img in minibatch:
            ...
            logo_data = transform_occl3(
                grads_value, start_points[indexs[i+count]], occl_sizes[indexs[i+count]], logo_data, count)
            ...
            if(args.greedy_stratage == 'random_fix' or args.greedy_stratage == 'sequence_fix'):
    ...
    if(iters % 5 == 0):
        print("iteration ", iters, ". diff between raw and adversarial", gray_angle_diff/len(imgs)
              * (180/math.pi), ". change time is,", change_times, ". bad_change_times,", bad_change_times)
    if(iters % 10 == 0):
        np.save('out/'+str(iters)+'new_logo.npy', logo)
        imsave('out/'+str(iters)+'new_logo.png',
               deprocess_image(logo, shape=(logo_height, logo_width, 3)))
output = open('out/'+"Output.txt", "w")
...
for i in range(len(imgs)):
    angle1 = model1.predict(imgs[i])[0]
    output.write(
        str(i)+" " + str(float(angle3[i]))+" "+str(float(angle1)) + "\n")
    gen_img_deprocessed = draw_arrow3(deprocess_image(imgs[i], shape=(img_cols, img_rows)), min(
        max(angle3[i], -math.pi/2), math.pi/2), angle1)
    imsave('out/'+str(i) + 'th_img.png', gen_img_deprocessed)
output.close()
