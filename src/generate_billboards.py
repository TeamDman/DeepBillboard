...
parser.add_argument('-greedy_stra', '--greedy_stratage',
                    ...
                    choices=['random_fix', 'sequence_fix', 'dynamic',
                             'hightest_fix'], default='dynamic')
...
with open(args.path + '/coordinates.txt') as f:
  for line in f:
    ...
    # TODO:TIMES 2!
    occl_sizes.append(
        [max(1, round((float(row[8]) - float(row[2])) * 100.0 / img_size[0]))
         * 2, max(1, round(
...
for img in imgs:
  angle_labels.append(model1.predict(img)[0])
...
logo_height = 400
if (args.op):
  logo[:, :] = gen_optimal(imgs, model1, angle_labels, start_points, occl_sizes)
...
def train(iteration):
  ...
  for iters in range(args.grad_iterations):
    ...
    for i in range(0, len(imgs), batch):
      ...
      this_diff = total_diff(tmp_imgs, model1, angle_labels)
    ...
    for i in range(len(imgs)):
      prediction = model1.predict(imgs[i])[0]
  ...
  for i in range(len(imgs)):
    ...
    out_image = draw_arrow3(deprocess_image(imgs[i]), draw_angle, prediction)
